...
interface SOOption { id: string; so_number: string; customers: { name: string } | null }
...
export function NewDeliveryNoteForm({ customers, items, salesOrders }: { customers: CustomerOption[]; items: ItemOption[]; salesOrders: SOOption[] }) {
  const [rows, setRows] = useState<LineRow[]>([{ itemId: null, soItemId: '', qty: '1', remarks: '' }])
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState('')

  const addRow = () => setRows(prev => [...prev, { itemId: null, soItemId: '', qty: '1', remarks: '' }])
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    fd.set('_so_item_ids', JSON.stringify(valid.map(r => r.soItemId || null)))
    fd.set('_qtys', JSON.stringify(valid.map(r => Number(r.qty) || 0)))
    fd.set('_remarks', JSON.stringify(valid.map(r => r.remarks)))

    try { await createDeliveryNote(fd) }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4 rounded-lg border border-slate-700 bg-slate-800 p-5">
        ...
        <div>
          <label className="mb-1 block text-sm text-slate-400">Sales Order (optional)</label>
          <select name="so_id" className="w-full rounded-lg border border-slate-600 bg-slate-700 px-3 py-2 text-sm text-white focus:border-blue-500 focus:outline-none">
            <option value="">None</option>
            {salesOrders.map(so => <option key={so.id} value={so.id}>{so.so_number} - {so.customers?.name}</option>)}
          </select>
        </div>
        <div>
          <label className="mb-1 block text-sm text-slate-400">Delivery Date</label>
          <input name="delivery_date" type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full rounded-lg border border-slate-600 bg-slate-700 px-3 py-2 text-sm text-white focus:border-blue-500 focus:outline-none" />
        </div>
        <div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-700">
                ...
                <th className="px-2 py-2 text-left font-medium text-slate-400">Remarks</th>
            ...
            <tbody>
              {rows.map((row, i) => (
                <tr key={i} className="border-b border-slate-700/50">
                  ...
                  <td className="px-2 py-2">
                    <input value={row.remarks} onChange={e => updateRow(i, 'remarks', e.target.value)} className="w-full rounded border border-slate-600 bg-slate-700 px-2 py-1.5 text-xs text-white focus:border-blue-500 focus:outline-none" />
                  </td>
                  <td className="px-2 py-2">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/admin/delivery-notes" className="rounded-lg bg-slate-700 px-6 py-2.5 text-sm text-white transition-colors hover:bg-slate-600">Cancel</Link>
      </div>
    </form>
  )
}
