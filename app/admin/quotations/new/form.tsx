...
export function NewQuotationForm({ customers, items }: { customers: CustomerOption[]; items: ItemOption[] }) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-4 rounded-lg border border-slate-700 bg-slate-800 p-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="mb-1 block text-sm text-slate-400">Quote Date</label>
            <input name="quote_date" type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full rounded-lg border border-slate-600 bg-slate-700 px-3 py-2 text-sm text-white focus:border-blue-500 focus:outline-none" />
          </div>
          <div>
            <label className="mb-1 block text-sm text-slate-400">Valid Until</label>
            <input name="valid_until" type="date" className="w-full rounded-lg border border-slate-600 bg-slate-700 px-3 py-2 text-sm text-white focus:border-blue-500 focus:outline-none" />
          </div>
        </div>
        <div>
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/admin/quotations" className="rounded-lg bg-slate-700 px-6 py-2.5 text-sm text-white transition-colors hover:bg-slate-600">Cancel</Link>
      </div>
    </form>
  )
}
